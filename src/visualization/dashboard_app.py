...
from config import DATA_DIR, OUTPUT_DIR, PROCESSED_DATA
...
def load_data():
    ...
    # Load clean panel
    clean_panel_path = PROCESSED_DATA["clean_panel"]
    if clean_panel_path.exists():
        data['clean_panel'] = pd.read_csv(clean_panel_path)
    
    # Load simulation results
    pickle_path = output_dir / 'simulation_results.pkl'
    ...
    uncertainty_path = output_dir / 'uncertainty_quantification.pkl'
    ...
    for scenario in ['baseline', 'low_investment', 'moderate_increase', 'high_investment', 'aggressive', 'optimal']:
        country_path = output_dir / f'country_effects_{scenario}.csv'
